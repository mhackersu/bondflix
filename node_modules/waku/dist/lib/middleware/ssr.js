...
export const ssr = (options)=>{
    ...
    return async (ctx, next)=>{
        const { devServer } = ctx;
        if (devServer && await devServer.willBeHandledLater(ctx.req.url.pathname)) {
            await next();
            return;
        }
        ...
        try {
            const htmlHead = devServer ? config.htmlHead : entries.dynamicHtmlPaths.find(([pathSpec])=>getPathMapping(pathSpec, ctx.req.url.pathname))?.[1];
            if (htmlHead) {
                const readable = await renderHtml({
                    ...
                    renderRscForHtml: async (input, searchParams)=>{
                        ctx.req.url.pathname = config.basePath + config.rscPath + '/' + encodeInput(input);
                    ...
                    ...devServer ? {
                        isDev: true,
                        getSsrConfigForHtml: (pathname, searchParams)=>devServer.getSsrConfigWithWorker({
                                config,
                                pathname,
                                searchParams
                            }, {
                ...
                if (readable) {
                    ...
                    ctx.res.body = devServer ? readable.pipeThrough(await devServer.transformIndexHtml(ctx.req.url.pathname)) : readable;
                    return;
                }
            }
        } catch (err) {
